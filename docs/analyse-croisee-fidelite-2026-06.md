...
**Recommandation d'ordre** : ~~**A + B**~~ → livrés ; ~~**D + G**~~ → **livrés 2026-06-03** (déterministes, indépendants). **C** *replay-gated* par le diagnostic Q3 (ne pas coder `exposure` à l'aveugle). **E2** garde sa part *cheap* (consigne MJ) faisable tôt ; sa part UI ainsi que **E1** et **F** après un replay live. **H** continu.

> **Suivi noté (symétrie, hors session D/G)** : le LOT D a fermé l'écart *persisté ≠ publié* pour le dialogue **PNJ** (`resolve_npc_dialogue`). Le **dialogue compagnon IA** (`narrative_flow_service.py:531/549`) publie via `publish_visible_entry` (donc nettoyé par `clean_dialogue_text`) mais **persiste le texte brut** → même classe de régression au rechargement. À traiter avec **E1** (chemin compagnon), pas dans le périmètre D/G.
