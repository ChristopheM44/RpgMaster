...
class TestOpenSession:
    async def test_open_returns_active_session(self, manager, session_in_db, db_session) -> None:
    ...
    async def test_open_registers_session(self, manager, session_in_db, db_session) -> None:
        await manager.open_session(session_in_db.id, db_session)
        assert manager.is_active(session_in_db.id)

    async def test_open_twice_is_idempotent(self, manager, session_in_db, db_session) -> None:
...
class TestGetSession:
    async def test_get_active_session(self, manager, session_in_db, db_session) -> None:
        await manager.open_session(session_in_db.id, db_session)
    ...
    async def test_active_session_ids(self, manager, session_in_db, db_session) -> None:
        await manager.open_session(session_in_db.id, db_session)
...
class TestTransitionPhase:
    async def test_valid_transition(self, manager, session_in_db, db_session) -> None:
        await manager.open_session(session_in_db.id, db_session)
    ...
    async def test_invalid_transition_raises(self, manager, session_in_db, db_session) -> None:
        await manager.open_session(session_in_db.id, db_session)
        with pytest.raises(InvalidTransitionError):
            await manager.transition_phase(session_in_db.id, SessionStatus.COMBAT, db_session)

    async def test_transition_inactive_session_raises(self, manager, db_session) -> None:
        with pytest.raises(KeyError):
            await manager.transition_phase("unknown", SessionStatus.LOBBY, db_session)

    async def test_transition_persists_to_db(self, manager, session_in_db, db_session) -> None:
        await manager.open_session(session_in_db.id, db_session)
...
class TestSaveState:
    async def test_save_marks_clean(self, manager, session_in_db, db_session) -> None:
        active = await manager.open_session(session_in_db.id, db_session)
        active.mark_dirty()
    ...
    async def test_save_inactive_session_is_noop(self, manager, db_session) -> None:
        # Should not raise
...
class TestCloseSession:
    async def test_close_removes_from_registry(self, manager, session_in_db, db_session) -> None:
        await manager.open_session(session_in_db.id, db_session)
