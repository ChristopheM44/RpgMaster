...
<template>
  <div
    class="rpg-border shrink-0 px-5 py-4 border-b"
  >
    <div class="rpg-eyebrow mb-2">✦ Carnet d'aventure</div>

    <div v-if="!journal || (!locationText && !journal.calendar_date)" class="rpg-text-muted font-display text-[15px] italic">
      Lieu inconnu…
    </div>

    <template v-else>
      <div class="rpg-text-main font-display text-[15px] font-semibold leading-snug">
        {{ locationText ?? 'Lieu inconnu' }}
      </div>
      <div class="rpg-text-dim mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px]">
